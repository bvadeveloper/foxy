...
public class DiffieHellmanKeyGenerator : IDisposable
{
    private readonly ECDiffieHellman _ecDiffieHellman;

    public byte[] PublicKey => _ecDiffieHellman.PublicKey.ExportSubjectPublicKeyInfo();
    ...
    public void Dispose() => _ecDiffieHellman.Dispose();

    public byte[] MakeDerivedSecret(byte[] publicKey)
    {
        using var ecdh = ECDiffieHellman.Create();
        ecdh.ImportSubjectPublicKeyInfo(publicKey, out _);
        return _ecDiffieHellman.DeriveKeyFromHash(ecdh.PublicKey, HashAlgorithmName.SHA256);
    }
}
