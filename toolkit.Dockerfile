...
RUN apt-get update \
    ...
    && wget http://http.kali.org/kali/pool/main/k/kali-archive-keyring/$KALI_KEYRING_DEB \
    && dpkg -i $KALI_KEYRING_DEB \
    && sh -c "echo 'deb http://http.kali.org/kali kali-rolling main non-free contrib' > /etc/apt/sources.list.d/kali.list" \
    && apt-get update
